  ...
  useEffect(() => {
    ...
    const fetchCategoryNames = async () => {
      try {
        ...
        const formattedContent = filteredData.map(({ PostID = '', PostType, PostContent, Category, PostTags, PostAuthor, PostDate, PostAuthorAvatar, PostImage }) => ({
          id: (PostID).toString(),
          type: PostType,
          content: PostContent,
          Category,
...
const CatList: React.FC<TagListProps> = ({ cats }) => (
  <div className="mt-6 flex flex-wrap">
    {cats.map(cat => (
      <span key={cat} className="px-4 py-2 bg-blue-500 text-white rounded-full text-sm mb-3 mr-3">{cat}</span>
    ))}
  </div>
);
